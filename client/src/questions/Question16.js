...
export default function Question16() {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("q16")) {
      setNum(JSON.parse(localStorage.getItem("q16")));
    }
  }, []);
  const rows =
    lng === "English"
      ? [
          ...
          {
            key: "C",
            value: "Engaging with or mentoring/developing employees",
          },
          {
            key: "D",
            value:
  ...
  useEffect(() => {
    ...
    localStorage.setItem("q16", JSON.stringify(num));
  }, [num]);
  ...
  function handleSubmit(e) {
    ...
    if (total !== 100) {
      ...
      const data = {
        ...
        q16: JSON.parse(localStorage.getItem("q16")),
      };
        ...
        {lng === "English" ? (
          <>
            <p className="question">
              During your work time, on average, what percentage of time do you
          ...
          <>
            <p className="left-align-text question">
              В течение рабочего времени, в среднем, какой процент времени Вы
              тратите на каждое из следующих действий?
            </p>
            <p className="question-i">
